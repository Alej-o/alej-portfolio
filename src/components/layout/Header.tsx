  ...
  >
    ...
    <div className="relative flex items-center w-full min-w-0 px-4 py-2 xl:hidden overflow-hidden">
      <button
        ...
        className={`w-10 h-10 flex items-center justify-center flex-shrink-0 group ${transitionClass} ${heroDisabled ? 'pointer-events-none cursor-default' : 'cursor-pointer'}`}
      ...
      <button
        ...
        className={`ml-auto uppercase font-title text-xl md:text-2xl lg:text-3xl ${textColorClass} ${transitionClass} hover:opacity-70 flex-shrink-0`}
      >
        {isOpen ? 'Fermer' : 'Menu'}
      </button>
    </div>
    ...
    <nav
      ...
      role="navigation"
      className="hidden xl:grid grid-cols-3 items-center px-6 py-4"
    >
      <div className="flex justify-start">
        <button
          onClick={handleHeroClick}
          disabled={heroDisabled}
          className={`font-title xl:text-2xl 2xl:text-4xl uppercase ${textColorClass} ${transitionClass} ${heroDisabled ? 'pointer-events-none cursor-default' : 'cursor-pointer'}`}
          tabIndex={heroDisabled ? -1 : 0}
          aria-label="Retour à l'accueil"
        >
          Agathe Lejour
      ...
      <div className={`text-center font-title lg:text-2xl xl:text-2xl 2xl:text-4xl uppercase ${textColorClass} ${transitionClass}`}>
        Portfolio 2025
      </div>
          ...
          return (
            <li key={label}>
              <FlipLink
                ...
                aria-current={pathname.includes(scrollTo || '') ? 'page' : undefined}
              >
                <span className={`lg:text-2xl xl:text-2xl 2xl:text-4xl ${textColorClass} ${transitionClass}`}>
                  {label}
                </span>
