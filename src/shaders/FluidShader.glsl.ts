...
void main(){
  ...
  float t=iTime*0.06;
  st += 0.05 * sin(vec2(st.y,st.x)*6.0 + t*uFlow);
  ...
  vec3 base = mix(uColor4, uColor3, 0.26);  
  vec3 col  = mix(uColor4, uColor1, n1);
  col = mix(col, uColor2, n2);
  col = mix(col, uColor3, n3);
  col = mix(col, uColor4, n4);
  ...
  float h1=exp(-pow(distance(uv,c1)*5.0,2.0));
  float h2=exp(-pow(distance(uv,c2)*4.2,2.0));
  col += (h1 + 0.6*h2) * uColor1 * 0.55;
